...
module.exports.addFriend = async function (req, res) {
  try {
    let user = req.user;
    let friend = await User.findById(req.params.id).populate({
      path: "friendships",
    });
    // console.log(friend);
    if (friend) {
      let index = friend.friendships.findIndex(
        (f) => f.from_user == user.id || f.to_user == user.id
      );
...
module.exports.removeFriend = async function (req, res) {
  ...
  // console.log(user);
  // console.log(friend);
  let friendship = friend.friendships.find(
    (f) => f.from_user == user.id || f.to_user == user.id
  );
