...
module.exports.sendEmail = function (req, res) {
  User.findOne({ email: req.body.email }, function (err, user) {
    if (err) {
      console.log("err in finding user-->reset password ", err);
      return;
    }
    ...
    if (user) {
      Reset.create(
        ...
        function (err, reset) {
          if (err) {
            console.log("err in creating schema for reset password ", err);
            return;
          }
          reset.user = user;
...
module.exports.newPassword = function (req, res) {
  ...
  Reset.findOne({ token: req.params.token }, function (err, reset) {
    if (err) {
      console.log("Error in newPassword controller, ", err);
      return;
    }
    console.log(reset);
  ...
  Reset.findOne({ token: req.params.token }, function (err, reset) {
    ...
    if (reset && reset.isValid) {
      User.findByIdAndUpdate(
        ...
        function (err, user) {
          if (err) {
            console.log("Error in updating password of user, ", err);
            return;
          }
          reset.isValid = false;
          reset.save();
